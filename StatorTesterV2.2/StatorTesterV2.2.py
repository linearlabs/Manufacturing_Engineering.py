...
configFolder = '//CTGTECH-LLDC01.linearlabs.local/Production/Production Data/Stator Config Files'
configFilePath = '//CTGTECH-LLDC01.linearlabs.local/Production/Production Data/Stator Config Files/folderConfig.txt'
...
def getPartParams(partNum, sn):                 # breaks down part number to determine variables needed
    ...
    try:
        ...
        configFile = partNum
        configParser = configparser.RawConfigParser()   
        configFilePath = '//CTGTECH-LLDC01.linearlabs.local/Production/Production Data/Stator Config Files/'+configFile+'.txt'
        ...
        label = tk.Label(fg="black",bg='red',width=25,font=16, text=comstat)
...
def isMCR5100(s):                                                   # if it's an MCR-5100 on the line
    ...
    idResponseLCR = 'XXXXX,,1.8.1\r\n'                              # response string
    idResponseLCR = idResponseLCR.encode()                          # convert to a byte array
...
def connectMCR5100(comPorts):                                       # try to locate an MCR-5100
    for port in comPorts:                                           # try each port in the list
        ...
        comstat='MCR5100 USB: Searching'
        label1 = tk.Label(fg="black",bg='white',width=25,font=16, text=comstat)
        ...
        if isMCR5100(s):                                            # if found,
            ...
            comstat='MCR5100 USB: '+str(s.port)
            label1 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text=comstat)
            label1.grid(row=0, column=2)
            label1.update()
...
def getDataMCR():                                                   # get data from the meter
    global mcr
    ...
    if len(result) != 3:                                         # if we don't have a good result
        if not mcr:                                              # check for no port
            ...
            mcr = connectMCR5100(comPorts)                          # connect to an MCR5100, if available
            sleep(1)
...
def checkMCR():
    ...
    if (R<=float(resMin)):                                                 # checks to make sure resistance isnt too LOW
        ...
        label = tk.Label(fg="black",bg='red',width=25,font=16, text=comstat)
        label.grid(row=index+2, column=2)
        label.update()
...
def testMCR():
    ...
    label4 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-B LCR: Relays On')
    ...
    if (status == 'A-B LCR') and (value == 1):                      # checks arduino reply and makes sure correct relays are on
        ...
        label4 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-B LCR: Relays Off')
        label4.grid(row=3, column=2)
        label4.update()
        ...
        table.append(row) 
        label4 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="A-B LCR: PASS")
        label4.grid(row=3, column=2)
        label4.update()
    if (status == 'A-B LCR') and (value == 0):                      # status is which phase to phase LCR test and value is if its on or off (1 or 0)
        ...
        label5 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-C LCR: Relays On')
    ...
    if (status == 'A-C LCR') and (value == 1):                      # status is which phase to phase LCR test and value is if its on or off (1 or 0)
        ...
        label5 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-C LCR: Relays Off')
        label5.grid(row=4, column=2)
        label5.update()
        sleep(1)                                                    # sleep to wait on relays to settle
        ...
        row.append('Pass')                                          # pass is writen in spread sheet, checkMCR() will kill program if it sees a failure
        table.append(row)
        label5 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="A-C LCR: PASS")
        label5.grid(row=4, column=2)
        label5.update()

    if (status == 'A-C LCR') and (value == 0):                      # status is which phase to phase LCR test and value is if its on or off (1 or 0)
        bclrON()
        label6 = tk.Label(fg="black",bg="white",width=25,font=16, text='B-C LCR: Relays On')
    ...
    if (status == 'B-C LCR') and (value == 1):                      # status is which phase to phase LCR test and value is if its on or off (1 or 0)
        ...
        label6 = tk.Label(fg="black",bg="white",width=25,font=16, text='B-C LCR: Relays Off')
        label6.grid(row=5, column=2)
        label6.update()
        sleep(1)                                                    # sleep to wait on relays to settle
        index = index + 1                                           # index counts up every mcr phase to phase test 1-3, when it = 4 Hpot will know its safe to try a test
        ...
        row.append('Pass')                                          # pass is writen in spread sheet, checkMCR() will kill program if it sees a failure
        table.append(row)
        label6 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="B-C LCR: PASS")
        label6.grid(row=5, column=2)
        label6.update()
...
def isGPT9803(s):                                                   # if it's a GPT-9803 on the line
    idResponseHpot = 'GPT-9803 ,GEU220453   ,V3.07,\r\n'            # response string
    idResponseHpot = idResponseHpot.encode()                        # convert to a byte 
    #sleep(1)
    s.flushInput()                                                  # make sure the input buffer is empty
    s.write(idQuery)                                                # send the ID command
    z = s.readline()                                                # response string
    return z == idResponseHpot

def connectGPT9803(comPorts):                                       # try to locate an GPT-9803
    for port in comPorts:                                           # try each port in the list
        comstat='GPT9803 USB: Searching'
        label2 = tk.Label(fg="black",bg='white',width=25,font=16, text=comstat)
        label2.grid(row=1, column=2)
        label2.update()                                               
        s = serial.Serial(port,baudrate=115200,timeout = 1)         # open port at 115200
        if isGPT9803(s):                                            # if found,
            print('\r\nGPT-9803 found on port ',s.port)
            comstat='GPT9803 USB: '+str(s.port)
            label2 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text=comstat)
            label2.grid(row=1, column=2)
            label2.update()
            return s                                                # return the port
        else:
...
def getDataGPT():                                                   # get data from the meter
    global gpt
    ...
    if len(result) != 3:                                         # if we don't have a good result
        if not gpt:                                              # check for no port
            ...
            print('.',end='',flush=True)                            # show we are searching
            comPorts = getPorts()                                   # get the list of available ports
            gpt = connectGPT9803(comPorts)                          # connect to an GPT9803, if available
            sleep(1)                                                # delay a bit
        try:                                                        # attempt to get data
...
def setUpGPT():
    ...
    if resultGPT == 'ACW':                                          # if recieved == send                             
        ...
        comStat='HiPot freq = '+resultGPT
        label7 = tk.Label(fg="black",bg='white',width=25,font=16, text=comStat)
    ...
    if resultGPT == x:                                              # if recieve == send (ERROR CHECK)
        ...
        comStat='HiPot voltage = '+resultGPT
        label7 = tk.Label(fg="black",bg='white',width=25,font=16, text=comStat)
        label7.grid(row=6, column=2)
        label7.update()
    ...
    if x == y:
        ...
        x=resultGPT
        print('Hpot lowset = '+x)
        comStat='HiPot lowset = '+resultGPT
        label7 = tk.Label(fg="black",bg='white',width=25,font=16, text=comStat)
        label7.grid(row=6, column=2)
        label7.update()
        x=resultGPT.strip('mA')
    ...
    if x == y:
        ...
        comstat= 'Hi Pot Parameter Error'
        label = tk.Label(fg="black",bg='red',width=25,font=16, text=comstat)
...
def testGPT():                                                          # combines func test and reading results of func test
    ...
    if (index == 4):
        ...
        if (index == 4):
            table = []
            hpotON()
            label7 = tk.Label(fg="black",bg='white',width=25,font=16, text='HiPot: Relays On')
            label7.grid(row=6, column=2)
            label7.update()
            ...
            hpotOFF()
            label7 = tk.Label(fg="black",bg='white',width=25,font=16, text='HiPot: Relays Off')
            label7.grid(row=6, column=2)
            label7.update()
    if (index !=4):
        print('Error.')                                      #index helps make sure correct relays are on for hi pot test
        comstat= 'HiPot: Index Error'
        label = tk.Label(fg="black",bg='red',width=25,font=16, text=comstat)
        label.grid(row=6, column=2)
        label.update()
    ...
    if (statusGPT == 'PASS '):
        ...
        index=5
        label7 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="HiPot: PASS")
        label7.grid(row=6, column=2)
        label7.update()
        print(dfGPT)
        return
...
def isStat001(s):                                                   # if it's a stat001 on the line
    idResponseStat = 'stator tester, test001, V2.0\r\n'             # response string
    idResponseStat = idResponseStat.encode()                        # convert to a byte array
    sleep(1.6)                                                      # arduino in Stator Tester takes a little over a second to respond
    s.flushInput()                                                  # make sure the input buffer is empty
    s.write(idQuery)                                                # send the ID command
...
def connectStat001(comPorts):                                       # try to locate a Stat001
    for port in comPorts:                                           # try each port in the list
        comstat= 'Tester USB: Searching'
        label3 = tk.Label(fg="black",bg='white',width=25,font=16, text=comstat)
        label3.grid(row=2, column=2)
        label3.update()                                                 
        s = serial.Serial(port,baudrate=115200,timeout = 1.5)       # open port at 115200
        if isStat001(s):                                            # if found,
            print('\r\nTester found on port ',s.port)
            comstat= 'Tester USB: '+str(s.port)
            label3 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text=comstat)
            label3.grid(row=2, column=2)
            label3.update()
            return s                                                # return the port
        else:
...
def getDataStat():                                                  # get data from the tester
    global sta                  
    result = ""                                                     # start with a bad result
    if len(result) != 3:                                         # if we don't have a good result
        if not sta:                                              # check for no port
            ...
            comPorts = getPorts()                                   # get the list of available ports
            sta = connectStat001(comPorts)                          # connect to a Stat001, if available
            sleep(1)                                                # delay a bit
        try:                                                        # attempt to get data
...
def staComms(query):                                                #used to parse biid data
    ...
    if status == 'homingY':                                         # next few if statements needed to fix a string to int bug
        ...
        label4 = tk.Label(fg="black",bg="red",width=25,font=16, text=failStatus1)
        label4.grid(row=3, column=2)
        label4.update()
...
def fluxTest():                                                     #B2 params
    ...
    if index==5: 
        ...
        if value >= 98:                                                 #if back iron result is greater than or equal to 98 then no back iron is detected
            ...
            label8 = tk.Label(fg="black",bg='red',width=25,font=16, text="BackIron missing/not seated.")
        ...
        if value==int(biidNum) or value==int(biidNum)+1 or value==int(biidNum)-1:                                                #if biid: responce from arduino is is within range of 48 to 50 then continue on!
            ...
            label8 = tk.Label(fg="black",bg='white',width=25,font=16, text=comstat)
            label8.grid(row=7, column=2)
            label8.update()
        else:                                                           #else print wrong back iron installed and exit flux test.
            print('\r\nWrong back iron installed.\r\n')
            label8 = tk.Label(fg="black",bg='red',width=25,font=16, text="Wrong BackIron")
            label8.grid(row=7, column=2)
            label8.update()
        ...
        print('\r\nWrong back iron installed.\r\n')
        label8 = tk.Label(fg="black",bg='red',width=25,font=16, text="Flux: Config Fault")
        label8.grid(row=7, column=2)
        label8.update()
...
def passFail():
    ...
    if failureTableHS1.empty:                                                                                       # if no failures found, test passes
        ...
        label8 = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text='Flux: PASS')
        label8.grid(row=7, column=2)
        label8.update()
        ...
        if not failureTableHS1.empty:                                                   # checks each if reading should exist and has a failure
            ...
            label8 = tk.Label(fg="black",bg='red',width=25,font=16, text="Flux: Failed")
            label8.grid(row=7, column=2)
            label8.update()
...
def fluxStatus(mag):
    comstat = 'Flux: PoleNumber='+str(mag)
    label8 = tk.Label(fg="black",bg="white",width=25,font =16, text=comstat)
    label8.grid(row=7, column=2)
    label8.update()
...
def fluxFailure(status):
    ...
    if status == 'setupY':                                          # status handling
        print(status)
        label8 = tk.Label(fg="black",bg="white",width=25,font=16, text='Flux: SetupY')
        label8.grid(row=7, column=2)
        label8.update()
    elif status == 'homingY':                                       # status handling
        print(status)
        label8 = tk.Label(fg="black",bg="white",width=25,font=16, text='Flux: HomingY')
        label8.grid(row=7, column=2)
        label8.update()
    elif status == 'homingX':                                       # status handling
        print(status)
        label8 = tk.Label(fg="black",bg="white",width=25,font=16, text='Flux: HomingX')
        label8.grid(row=7, column=2)
        label8.update()
    elif status == 'failedHomingY':                                 # failure handling
        print(status)
        label8 = tk.Label(fg="black",bg="red",width=25,font=16, text='Flux: Failed HomingY')
        label8.grid(row=7, column=2)
        label8.update()
        failStatus1 = 'Failed Homing Y'
        ...
        print(status)
        label8 = tk.Label(fg="black",bg="red",width=25,font=16, text='Flux: Failed HomingX')
        label8.grid(row=7, column=2)
        label8.update()
        failStatus1 = 'Failed Homing X'
...
def dataCollection(partNum, sn):
    ...
    if mag != int(magNum)+1:
        ...
        print (failStatus1)
        print (failStatus2)
        label = tk.Label(fg="black",bg='red',width=25,font=16, text="Failed Flux Test")
        label.grid(row=0, column=0)
        label.update()
        label8 = tk.Label(fg="black",bg='red',width=25,font=16, text="Flux: Test Failed")
        label8.grid(row=7, column=2)
        label8.update()
        index=0
        failure()
    label = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="Enter Barcode")
    label.grid(row=0, column=0)
    label.update()
...
def about():
    print('\n--About the program--')
    print('Stator Tester GUI')
    print('Author: Doug Gammill')
    print('Date: 9/15/2021')
    ...
    print('1. Turn on MCR-5100 (LCR meter), and GPT-9803 (hi pot meter).')
    print('2. Turn on Stator tester power switch on the back and then the power supply LCD located on the front.')
    print('3. Ensure all USB cables for each device are connected to the laptop')
    print('4. Ensure Linear Labs Stator Tester computer application (gui) has indicated USB coms are good with each device.')
    print('5. Place the desired part into the test fixture. Make sure to use the correct adapter for the part.')
    print('6. Scan the barcode located in the part folder using the barcode scanner.')
    print('7. Confirm part number selected is correct')
    print('8. Confirm that there are no failure(s) for the part by reading the label outputs.')
    print('9. Check the excel output file if desired.')
    print('10. excel output file location: (O):\Manufacturing Engineering\Stator Test\Stator Test Results')
    window = tk.Tk()
    window.title("About")
    ...
    Date: 10/31/2021
    Copyright (c) 2021
    Linear Labs, Inc.    label8 = tk.Label(fg="black",bg="white",width=25,font=16, text='Flux:')
    label8.grid(row=7, column=2)
    label8.update()
    Tests a Stator for flux strength and polarity for each of its poles.
    Conducts a phase to phase inductance and resistance test for all three phases.
    ...
    9. Check the excel output file if desired.
    10. Excel output file location: (O):\Manufacturing Engineering\Stator Test\Stator Test Results
    '''
...
def failure():
    ...
    def reset():
        ...
        label4 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-B LCR:')
        label4.grid(row=3, column=2)
        label4.update()

        label5 = tk.Label(fg="black",bg="white",width=25,font=16, text='A-C LCR:')
        label5.grid(row=4, column=2)
        label5.update()

        label6 = tk.Label(fg="black",bg="white",width=25,font=16, text='B-C LCR:')
        label6.grid(row=5, column=2)
        label6.update()

        label7 = tk.Label(fg="black",bg="white",width=25,font=16, text='HiPot:')
        label7.grid(row=6, column=2)
        label7.update()

        label8 = tk.Label(fg="black",bg="white",width=25,font=16, text='Flux:')
        label8.grid(row=7, column=2)
        label8.update()
        index=0
        barcode_var.set('')
    ...
    failLabel = tk.Label(fail, fg="black",bg='red',width=80, font=("Adobe",16), text='Enter additional notes and save or reset to start again.')
    failLabel.grid(row=1, column=1)

    failLabel1 = tk.Label(fail, fg="black",bg='red',width=80, font=("Adobe",16), text=failStatus1)
    failLabel1.grid(row=2, column=1)
    
    failLabel2 = tk.Label(fail, fg="black",bg="red",width=80, font=("Adobe",16), text=failStatus2)
    failLabel2.grid(row=3, column=1)
    
    fr_buttons = tk.Frame(fail)
    btn_reset = tk.Button(fr_buttons,font=("Adobe",16), text="Reset", command=reset)
    btn_save = tk.Button(fr_buttons,font=("Adobe",16), text="Save", command=save_file)
...
def getPartInfo(event):                                             #event is used for gui entry .bind <return>
    global barcode, partNum, sn, comOK
    #print('\n------Scan Barcode------')
    if comOK==1:
        try:
            ...
            if len(barcode.split('-')) == 4:                        # check if the barcode is the correct length/format
                ...
                label = tk.Label(fg="black",bg='Yellow',width=25,font=16, text=status)
                label.grid(row=0, column=0)
                label.update()
                dataCollection(partNum, sn)
                                                                                          # if correct format, do nothing
            else:
...
def startpress():
    ...
    try:
        getDataMCR()                                            # get MCR COM port
    except:
        label1.config(bg='red', text='MCR5100 USB: Error')
    try:
        getDataGPT()                                            # get GPT COM port
    except:
        label2.config(bg='red', text='MCR5100 USB: Error')
    try:       
        getDataStat()                                           # get Arduino COM port
        label = tk.Label(fg="black",bg='#3399ff',width=25,font=16, text="Enter Barcode")
        label.grid(row=0, column=0)
        label.update()
        comOK=1
        ...
        label3.config(bg='red', text='MCR5100 USB: Error')
...
if __name__ == "__main__":
    ...
    gui.geometry("900x500") #col x row
        
    gui.resizable(width=False, height=False)
          
    label = tk.Label(fg="white",bg="red",width=25,font=16, text="Do Not Enter Barcode Yet")
    label.grid(row=0, column=0)
    label.update()

    barcode_var = tk.StringVar()
    expression_field = tk.Entry(gui,width=50, font=("Adobe",16), textvariable=barcode_var)
    expression_field.grid(row=1, column=0)
    expression_field.bind('<Return>', getPartInfo)
    ...
    label1 = tk.Label(fg="black",bg='white',width=25,font=16, text='MCR5100 USB: ')
    label1.grid(row=0, column=2)
    label1.update()
    
    label2 = tk.Label(fg="black",bg="white",width=25,font=16, text='GPT9803 USB: ')
    label2.grid(row=1, column=2)
    label2.update()

    label3 = tk.Label(fg="black",bg='white',width=25,font=16, text='Tester USB: ')
    label3.grid(row=2, column=2)
    label3.update()
