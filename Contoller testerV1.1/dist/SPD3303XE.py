...
comPort1 = configParser.get('folderPath', 'comPortSPD3303X-E')
comPort2 = configParser.get('folderPath', 'comPortSPD3303X-E2')
idResponse1 = configParser.get('folderPath', 'idResponseSPD3303X-E')
#print(idResponse1)
idResponse2 = configParser.get('folderPath', 'idResponseSPD3303X-E2')
#print(idResponse2)
...
if comPort1 in s:   
    print(comPort1)
    my_instrument = rm.open_resource(comPort1)
    
elif comPort2 in s:
    print(comPort2)
    my_instrument = rm.open_resource(comPort2)
else:
    passFail=1


def connectPowerSupply():
    global rm, my_instrument,passFail
    if passFail==0:
        x = my_instrument.query('*IDN?')
        x=x.strip()
        print(x)
        if x == idResponse1 or x == idResponse2:
            print("SPD3303X-E Power Supply connected")
        else: passFail=1
    else:
        passFail=1
        print("Power Supply USB Failed")
    return passFail
