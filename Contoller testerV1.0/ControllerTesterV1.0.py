...
def readConfig():
    global passFail,partNum,status,sourceMode,sourceVoltage,sourceCurrent,ch2,minI0,maxI0,minI50,maxI50,minI100,maxI100,minRPM50,maxRPM50,minRPM100,maxRPM100,minV,maxV
    # get partNum params
    configFile = partNum
...
def checkKeyThrottle3V3():
    global passFail,minV,maxV,img2,img3,img4,img5,img6,speed0,speed1,speed2,volt3V3,failureMessage, failureMessage2
    minV=float(minV)
    maxV=float(maxV)
    ...
    if speed0 == '0.00':
        arduino.command('spee:10') # set speed to check key
        sleep(.5)
...
def checkFwd50():
    global passFail,minI50,maxI50,minRPM50,maxRPM50,img7,img8,img9,currentFwd50,speedFwd50,directionFwd50,failureMessage,failureMessage2
    minI50=float(minI50)
    maxI50=float(maxI50)
    minRPM50=float(minRPM50)
    maxRPM50=float(maxRPM50)
    arduino.command('spee:40') # set throttle back to Zero
    sleep(3)
...
def checkFwd100():
    global passFail,minI100,maxI100,minRPM100,maxRPM100,img10,img11,img12,currentFwd100,speedFwd100,directionFwd100,failureMessage,failureMessage2
    minI100=float(minI100)
    maxI100=float(maxI100)
    minRPM100=float(minRPM100)
    maxRPM100=float(maxRPM100)
    arduino.command('spee:80') # set throttle back to Zero
    sleep(3)
    SPD3303XE.ch1MeasureCurrent()
...
def checkRev50():
    global passFail,minI50,maxI50,minRPM50,maxRPM50,img13,img14,img15,currentRev50,speedRev50,directionRev50,failureMessage,failureMessage2
    minI50=float(minI50)
    maxI50=float(maxI50)
    minRPM50=float(minRPM50)
    maxRPM50=float(maxRPM50)
...
def checkRev100():
    global passFail,minI100,maxI100,minRPM100,maxRPM100,img16,img17,img18,currentRev100,speedRev100,directionRev100,failureMessage,failureMessage2
    minI100=float(minI100)
    maxI100=float(maxI100)
    minRPM100=float(minRPM100)
    maxRPM100=float(maxRPM100)
...
def mainFunctions(): # main multi threaded functions
    global passFail, failureMessage,failureMessage2,status
    passFail=1
    resetGui()
    getBarcodeInfo()
    if passFail==1:
...
usb1=tk.Label(gui, text="USB1", height=3, width=16, font='12', fg='black', bg='white', relief="groove", borderwidth=2)
usb1.grid(row=0, column=3)
usb1.update()
usb2=tk.Label(gui, text="USB2", height=3, width=16, font='12', fg='black', bg='white', relief="groove", borderwidth=2)
