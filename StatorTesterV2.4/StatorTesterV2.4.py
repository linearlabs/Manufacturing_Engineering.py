...
import configparser
import StatorTesterOneFactory # Save results in One factory format
import warnings # python module for disabling warnings
...
saveFolder = configParser.get('folderPath', 'savePath')
...
def getPartParams(partNum, sn):                 # breaks down part number to determine variables needed
    global magNum,yAxis,aPol,cPol,initPole,biidNum,mcrFreq,resMin,resMax,indMin,indMax,mode,gptV,gptI,gptImin,gptFreq,failStatus1,failStatus2,index,poleStrengthMinimum,poleStrengthMaximum,pOff,sOff
    ...
    try:
        ...
        if Path(configFilePath).is_file():
            ...
            poleStrengthMaximum = configParser.get(configFile, 'poleStrengthMaximum')
            #print ('poleStrengthMaximum='+poleStrengthMaximum)
...
def getPassFail(HSVal, actualPole, expectedPole):
    global poleStrengthMinimum, poleStrengthMaximum
                                                                    
    if HSVal < float(poleStrengthMinimum) or HSVal > float(poleStrengthMaximum): # desired cutoff for a "strong" magnet, adjust this value as desired
...
def passFail():
    global df, dfGPT, dfMCR, barcode, failStatus1, failStatus2
    ...
    if failureTableHS1.empty: # if no failures found, test passes
        ...
        fullPath = pathlib.Path(os.path.join('temp/', file + extension))
        print(fullPath)
        ...
        writer.close()
        writer.handles = None
        StatorTesterOneFactory.saveOneFactory()
    else:
...
def open_file():
    global saveFolder
    filename = filedialog.askopenfilename(initialdir=saveFolder, title="Stator Test Results",filetypes=(("xlsx files", "*.xlsx"),("all files", "*.*")))
    filename = '"'+filename+'"'
    if len(filename) >=3:
...
def failure():
    global failStatus1, failStatus2, barcode, df, dfMCR, dfGPT, index
    fail = tk.Tk()
    ...
    def save_file():
        ...
        extension = '.xlsx'                                                         # file extension
        fullPath = pathlib.Path(os.path.join('temp/', file + extension))# the full file path
        print(fullPath)
        writer = pd.ExcelWriter(path = fullPath, engine='xlsxwriter',engine_kwargs={'options': {'strings_to_formulas': False}})       
